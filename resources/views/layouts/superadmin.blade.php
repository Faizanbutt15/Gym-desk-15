...
<body class="bg-gray-50 text-gray-900 font-sans antialiased flex h-screen overflow-hidden" x-data="{ mobileMenuOpen: false }">
    ...
    <aside class="w-64 bg-primary text-white flex flex-col hidden md:flex absolute md:relative z-20 h-full transition-transform duration-300 transform md:translate-x-0" :class="mobileMenuOpen ? 'translate-x-0 !flex' : '-translate-x-full'">
        <div class="h-16 flex items-center justify-between px-6 font-bold text-2xl border-b border-blue-800 tracking-tight">
            GymOS
        ...
        <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
            <a href="{{ route('superadmin.dashboard') }}" class="block px-4 py-3 rounded-lg hover:bg-blue-800 transition {{ request()->routeIs('superadmin.dashboard') ? 'bg-blue-800 font-semibold shadow-inner' : '' }}">Dashboard</a>
            <a href="{{ route('superadmin.gyms.index') }}" class="block px-4 py-3 rounded-lg hover:bg-blue-800 transition {{ request()->routeIs('superadmin.gyms.*') ? 'bg-blue-800 font-semibold shadow-inner' : '' }}">Gyms</a>
            <a href="{{ route('superadmin.gym-admins.index') }}" class="block px-4 py-3 rounded-lg hover:bg-blue-800 transition {{ request()->routeIs('superadmin.gym-admins.*') ? 'bg-blue-800 font-semibold shadow-inner' : '' }}">Gym Admins</a>
            <a href="{{ route('superadmin.revenue') }}" class="block px-4 py-3 rounded-lg hover:bg-blue-800 transition {{ request()->routeIs('superadmin.revenue') ? 'bg-blue-800 font-semibold shadow-inner' : '' }}">Revenue</a>
        </nav>
    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden w-full">
        ...
        <header class="h-16 bg-white shadow-sm flex items-center justify-between px-6 md:justify-end border-b border-gray-200">
            ...
            <div class="flex items-center space-x-4">
                <span class="text-sm font-medium text-gray-500 hidden md:inline-block">Super Admin</span>
