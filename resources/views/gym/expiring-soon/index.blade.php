                ...
                <div class="p-5 flex flex-col h-full">
                    <div class="flex items-start justify-between mb-4">
                        <div class="flex items-center gap-4">
                            @if($member->photo)
                                <img src="{{ asset('storage/' . $member->photo) }}" class="w-14 h-14 rounded-full object-cover border border-gray-200 shadow-sm">
                            @else
                                <div class="w-14 h-14 rounded-full bg-blue-100 text-primary flex items-center justify-center font-bold text-xl border border-blue-200 shadow-sm">
                                    {{ substr($member->name, 0, 1) }}
                                </div>
                            @endif
                            <div>
                                <h3 class="font-bold text-gray-900 text-lg">{{ $member->name }}</h3>
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold border mt-1 {{ $badgeColor }}">
                                    {{ $badgeText }}
                                </span>
                            </div>
                        </div>
                    ...
                    <div class="mt-auto">
                        <form method="POST" action="{{ route('members.markPaid', $member) }}" class="block w-full">
                            @csrf
                            <button type="submit" class="w-full text-white bg-green-500 hover:bg-green-600 py-2.5 rounded-lg text-sm font-bold uppercase tracking-wide transition shadow-sm text-center flex justify-center items-center gap-2" onclick="return confirm('Mark as Paid? This will extend the due date by 30 days and record a payment.')">
