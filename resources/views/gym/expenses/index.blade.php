...
<div class="space-y-4 md:space-y-6" x-data="{ addModalOpen: false, editModalOpen: false, editExpense: {} }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
            <h1 class="text-xl md:text-2xl font-bold text-zinc-100">Gym Expenses</h1>
        ...
        @foreach($categoryTotals as $cat)
            ...
            <div class="bg-zinc-900 border border-zinc-800 rounded-xl p-4 flex items-center gap-3">
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-lg font-bold text-zinc-100">${{ number_format($cat->total, 2) }}</p>
                </div>
            </div>
        @endforeach

        {{-- Grand Total --}}
        <div class="bg-red-900/20 border border-red-900/40 rounded-xl p-4 flex items-center gap-3 col-span-2 sm:col-span-1">
            <div class="w-10 h-10 rounded-xl bg-red-900/30 border border-red-900/50 flex items-center justify-center shrink-0">
            ...
            <div class="min-w-0">
                ...
                <p class="text-lg font-bold text-red-400">${{ number_format($grandTotal, 2) }}</p>
            </div>
        </div>
    </div>
    ...
    <form method="GET" action="{{ route('expenses.index') }}" class="flex flex-wrap gap-2">
        <select name="category" class="bg-zinc-900 border border-zinc-800 text-zinc-300 rounded-lg text-sm px-3 py-2 focus:ring-red-500 focus:border-red-500" onchange="this.form.submit()">
            <option value="all">All Categories</option>
            @foreach($categories as $cat)
                <option value="{{ $cat }}" {{ request('category') === $cat ? 'selected' : '' }}>{{ $cat }}</option>
            @endforeach
        </select>
        <select name="month" class="bg-zinc-900 border border-zinc-800 text-zinc-300 rounded-lg text-sm px-3 py-2 focus:ring-red-500 focus:border-red-500" onchange="this.form.submit()">
            <option value="">All Months</option>
            @foreach(range(1, 12) as $m)
                <option value="{{ $m }}" {{ request('month') == $m ? 'selected' : '' }}>{{ date('F', mktime(0,0,0,$m,1)) }}</option>
            @endforeach
        </select>
        <select name="year" class="bg-zinc-900 border border-zinc-800 text-zinc-300 rounded-lg text-sm px-3 py-2 focus:ring-red-500 focus:border-red-500" onchange="this.form.submit()">
            @foreach($years as $yr)
                <option value="{{ $yr }}" {{ (request('year', now()->year)) == $yr ? 'selected' : '' }}>{{ $yr }}</option>
            @endforeach
        </select>
        @if(request()->hasAny(['category','month','year']))
            <a href="{{ route('expenses.index') }}" class="px-3 py-2 text-sm text-zinc-400 hover:text-white border border-zinc-700 rounded-lg transition">Clear</a>
        @endif
    </form>
    ...
    <div class="bg-zinc-900 rounded-xl border border-zinc-800 overflow-hidden">
                        ...
                        <tr class="hover:bg-zinc-800/30 transition">
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="text-zinc-100 font-medium">{{ $expense->title }}</span>
                                </div>
                            </td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 text-zinc-400 whitespace-nowrap hidden md:table-cell">
                                {{ $expense->expense_date->format('M d, Y') }}
                            </td>
                            <td class="px-4 py-3 text-zinc-500 text-xs hidden lg:table-cell max-w-xs truncate">
                                {{ $expense->notes ?? '—' }}
                            </td>
                            <td class="px-4 py-3">
        ...
        <div class="px-4 py-4 border-t border-zinc-800">{{ $expenses->links() }}</div>
        @endif
    </div>
    ...
    <div x-show="addModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-cloak>
        ...
        <div class="relative bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-md shadow-2xl z-10">
            <div class="flex items-center justify-between px-6 py-4 border-b border-zinc-800">
                <h2 class="text-base font-bold text-zinc-100">Add Expense</h2>
            ...
            <form method="POST" action="{{ route('expenses.store') }}" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-semibold text-zinc-400 mb-1.5 uppercase tracking-wide">Title *</label>
                        <input type="text" name="title" required placeholder="e.g. Monthly Rent"
                               class="w-full bg-zinc-800 border border-zinc-700 text-zinc-100 rounded-lg px-3 py-2.5 text-sm focus:ring-red-500 focus:border-red-500 placeholder-zinc-600">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-zinc-400 mb-1.5 uppercase tracking-wide">Category *</label>
                        <select name="category" required class="w-full bg-zinc-800 border border-zinc-700 text-zinc-100 rounded-lg px-3 py-2.5 text-sm focus:ring-red-500 focus:border-red-500">
                            @foreach($categories as $cat)
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-zinc-400 mb-1.5 uppercase tracking-wide">Amount *</label>
                        <div class="relative">
                            <span class="absolute inset-y-0 left-3 flex items-center text-zinc-400 text-sm">$</span>
                            <input type="number" name="amount" step="0.01" min="0.01" required placeholder="0.00"
                                   class="w-full bg-zinc-800 border border-zinc-700 text-zinc-100 rounded-lg pl-7 pr-3 py-2.5 text-sm focus:ring-red-500 focus:border-red-500">
                        </div>
                    </div>
                    <div class="sm:col-span-2">
                        ...
                        <input type="date" name="expense_date" required value="{{ date('Y-m-d') }}"
                               class="w-full bg-zinc-800 border border-zinc-700 text-zinc-100 rounded-lg px-3 py-2.5 text-sm focus:ring-red-500 focus:border-red-500">
                    </div>
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-semibold text-zinc-400 mb-1.5 uppercase tracking-wide">Notes</label>
                        <textarea name="notes" rows="2" placeholder="Optional description..."
                                  class="w-full bg-zinc-800 border border-zinc-700 text-zinc-100 rounded-lg px-3 py-2.5 text-sm focus:ring-red-500 focus:border-red-500 placeholder-zinc-600 resize-none"></textarea>
                    </div>
                </div>
                <div class="flex gap-3 pt-2">
                    <button type="button" @click="addModalOpen = false" class="flex-1 px-4 py-2.5 rounded-lg border border-zinc-700 text-zinc-400 hover:text-white hover:border-zinc-500 text-sm font-semibold transition">Cancel</button>
    ...
    <div x-show="editModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-cloak>
        ...
        <div class="relative bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-md shadow-2xl z-10">
            <div class="flex items-center justify-between px-6 py-4 border-b border-zinc-800">
                <h2 class="text-base font-bold text-zinc-100">Edit Expense</h2>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea name="notes" rows="2" :value="editExpense.notes ?? ''"
                                      class="w-full bg-zinc-800 border border-zinc-700 text-zinc-100 rounded-lg px-3 py-2.5 text-sm focus:ring-red-500 focus:border-red-500 resize-none"></textarea>
                        </div>
                    </div>
                    <div class="flex gap-3 pt-2">
                        <button type="button" @click="editModalOpen = false" class="flex-1 px-4 py-2.5 rounded-lg border border-zinc-700 text-zinc-400 hover:text-white hover:border-zinc-500 text-sm font-semibold transition">Cancel</button>
